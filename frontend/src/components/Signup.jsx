...
const Signup = () => {
  ...
  return (
    <div className="signUpForm bg-black mx-auto mt-24 rounded-lg border-white shadow-lg shadow-stone-900">
      <h1 className="p-6 font-bold font-sans text-4xl" align="center">
        Sign Up
      </h1>
      <p className="place-self-center text-lg font-sans font-semibold text-stone-400 pb-5">Enter your information to create an account</p>
      <Form method="post" action="/signup" className="font-semibold w-11/12 font-sans text-left flex flex-col gap-4 mx-auto">
        <label htmlFor="firstName" className="w-full">First Name</label>
        <input type="text" name="firstName" required onChange={handleChange} className={`${inputClass}`}/>
        <label htmlFor="lastName" className="w-full">Last Name</label>
        <input type="text" name="lastName" required onChange={handleChange} className={`${inputClass}`}/>
        <label htmlFor="username" className="w-full">Username</label>
        <input type="text" name="username" required onChange={handleChange} className={`${inputClass}`} />
        ...
        <p className="mt-1 place-self-center mb-6">
          Already have an account? <Link to={"/"} className="underline underline-offset-2 hover:text-teal-500">Login</Link>
        </p>
      </Form>
    </div>
  );
